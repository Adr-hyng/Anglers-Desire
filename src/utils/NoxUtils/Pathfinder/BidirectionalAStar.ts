...
export class BidirectionalAStar{
    ...
    public *IterateUntilGoalBlockReached(goalNodePromiseResolve: (goalNode: IAStarNode[]) => void) {
        ...
        while (openList.length > 0 && reverseOpenList.length > 0) {
            ...
            const locationHash: string = Vec3.toString(nextNode.Block.location);
            ...
            const reverseLocationHash: string = Vec3.toString(reverseNextNode.Block.location);
            ...
            const reverseSurroundingLocations: Vector3[] = CuboidRegion.FromCenterLocation(reverseNextNode.Block.location, 1, true).GetAllLocationsInRegion();
            ...
            for (const surroundingBlock of surroundingBlocks) {
                ...
                const surroundingBlockLocationHash: string = Vec3.toString(surroundingBlock.location);
            ...
            for (const reverseSurroundingBlock of reverseSurroundingBlocks) {
                ...
                const reverseSurroundingBlockLocationHash: string = Vec3.toString(reverseSurroundingBlock.location);
    ...
    private SetDebugBlock(block: Block) {
        ...
        if(!(Vec3.toString(block.location) 
            in 
            [...this.TotalDebugBlocksCreated].map((b: Block)=> Vec3.toString(b.location)))
    ...
    private ClearDebugBlocks() {
      ...
      for(const v of locs){
        system.run(() => {
            const vx = v.location;
            if(Vec3.equals(vx, this.Options.StartLocation) || Vec3.equals(vx, this.Options.GoalLocation)) return
