...
export class Configuration {
                ...
                case 4:
                    ...
                    this.player.runCommandAsync(`testfor @s[hasItem={item=${addonConfigItemType.id}}]`).then((result) => {
                        if (!result.successCount) {
                            const inventory = this.player.getComponent(EntityInventoryComponent.componentId).container.override(this.player);
    ...
    showEnhancementInfoScreen() {
        ...
        form.show(this.player).then((response) => {
            if (response.canceled || response.cancelationReason === FormCancelationReason.UserClosed || response.cancelationReason === FormCancelationReason.UserBusy)
                return;
            if (response.selection >= AvailableCustomEnchantments.length)
            ...
            descriptionForm.show(this.player).then((descriptionResponse) => {
                ...
                if (descriptionResponse.selection === 0)
                    return this.showEnhancementInfoScreen();
                return;
            });
            return;
        });
    ...
    showConfigurationScreen() {
        system.run(() => {
            if (this.isConfigurationSettingsOpen)
                return;
            ...
            if (this.player.hasTag("isOperator")) {
                form.button({ rawtext: [
                    ...
                    ] }, "textures/gui/configurations/operator_status");
            }
            form.button({ rawtext: [
    ...
    showServerScreen() {
        ...
        Object.values(serverConfigurationCopy).forEach((builder, index) => {
            ...
            if (typeof builder.defaultValue === "string" && isArrayEmpty) {
                ...
                form.dropdown({ rawtext: [{ translate: builder.name }] }, builder.values, cachedConfigurationValues[index]);
            }
